...
\- It works with the network fully disconnected — this is enforced, not
  ...
  just documented: \`tests/test\_security.py\` runs the full
...
\- Loading a saved graph is pure data deserialization: nothing derived from
  ...
  \`graph/storage.py\` and \`tests/test\_security.py\`).
...
lcg build \<repo>            repository -> scanner -> language parsers -> GraphBuilder -> save\_graph()

lcg \<any other command>      .local-code-graph/graph.json -> load\_graph() -> QueryEngine -> result
...
implicitly. This is tested directly: \`tests/test\_cli.py\` builds a graph,
...
Alternatively, follow the [official `uv` installation instructions](https://docs.astral.sh/uv/getting-started/installation/).

If you installed `uv` with its standalone installer and your shell cannot find it, add its bin directory to your current shell session:

```bash
source "$HOME/.local/bin/env"
```

Clone the repository, enter its directory, and set up the project:
...
`uv python pin 3.11` records the project's Python version in `.python-version`.
Check which interpreter the project is using:

```bash
uv run python --version
```

It should report Python 3.11.x. If it reports a different version, check the project's Python configuration before continuing.

### Linux / other platforms

Install Python 3.11+ and `uv` using the instructions for your platform, then run:
...
lives in \`001\_init.sql\` and the column was added in \`014\_add\_body.sql\`.
...
\`graph/builder.py::default\_parsers()\` — the graph model, storage, query
...
├── graph.json         the graph itself: schema\_version, root, nodes, edges

├── metadata.json      informational: generator version, counts, languages,

│                       a generation timestamp (never in graph.json — see below)

└── file\_hashes.json   per-file content hashes, written by \`build\`/\`update\` so
...
\`src/local\_code\_graph/graph/model.py\`'s module docstring.
...
\- **\*\*Accuracy over completeness.\*\*** \`EXTENDS\`/\`IMPLEMENTS\`/\`IMPORTS\` targets
  ...
  and ambiguous references are left unresolved (\`target\_id: null\`) with
...
\- \*\*Deeply nested files used to be skipped; this is fixed, and the fix
  ...
  3\. \*\*\`tree-sitter\` 0.26.0 genuinely segfaults while walking a deeply
     ...
     without re-running \`tests/test\_deep\_nesting.py\`.
  ...
  \`src/local\_code\_graph/parser/isolated.py\` and

  \`src/local\_code\_graph/parser/\_ts\_utils.py\`.
...
\- **\*\*One known Tree-sitter grammar quirk:\*\*** in the current
  ...
  \`tests/test\_kotlin\_parser.py\`; not worked around, per this project's
