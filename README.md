...
- **One known Tree-sitter grammar quirk:** in the current
  ...
  preceding `annotation class Foo` (e.g. `@Target(...)` followed by
  `annotation class Foo`) is misparsed as an expression rather than a
  declaration, with no syntax error reported — the annotation class is
  silently absent from the graph in that specific case. Reproduced and
  regression-tested in `tests/test_kotlin_parser.py`; not worked around,
  per this project's policy against grammar-quirk hacks.
